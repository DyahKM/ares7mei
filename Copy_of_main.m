% Corrected main.m with Automatic Ratio Selection Based on RMSE + γ·Ratio
...
for i = 1:length(stops)
    ...
    fprintf('  → Ratio %2d: Final RMSE = %.4f | Cost = %.4f\n', ratio, ArgRMSE(end), costs(i));
end
    ...
    for j = 1:size(reports, 1)
        ...
        while sum(month_days_int) ~= target_sum
            if sum(month_days_int) < target_sum
                ...
                for k = 1:target_sum - sum(month_days_int)
                    month_days_int(idx(mod(k-1, length(idx))+1)) = month_days_int(idx(mod(k-1, length(idx))+1)) + 1;
                end
            else
                % Need to subtract more
                [~, idx] = sort(month_days_int, 'descend');
                for k = 1:sum(month_days_int) - target_sum
